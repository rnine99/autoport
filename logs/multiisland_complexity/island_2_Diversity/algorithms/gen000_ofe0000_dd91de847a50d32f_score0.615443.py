...
def select_ports(K, N_selected, N_Ports, Pt, n, H, noise):
   ...
   Select N_selected out of N_Ports ports to maximize the average objective value of n communications channels.
    ...
    for i in range(n):
        candidate_set = set()
        for k in range(K):
            port_gains = np.linalg.norm(H[i, :, k], axis=-1)
            top_ports = np.argsort(port_gains)[-K:]
            candidate_set.update(top_ports)
        available = list(candidate_set)
        if len(available) < N_selected:
            all_ports = list(range(N_Ports))
            additional = [p for p in all_ports if p not in available]
            available.extend(additional[:N_selected - len(available)])
        selected = []
        for _ in range(N_selected):
            best_port = -1
            best_value = -np.inf
            for port in available:
                candidate_ports = selected + [port]
                h_eff = H[i, candidate_ports, :]
                f_val = sinr_balancing_power_constraint(len(candidate_ports), K, h_eff, Pt, noise)
                if f_val > best_value:
                    best_value = f_val
                    best_port = port
