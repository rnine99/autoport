    ...
    for i in range(n):
        ...
        while len(selected) < N_selected:
            ...
            for port in remaining:
                ...
                h_candidate = channel[candidate_set, :]
                val = sinr_balancing_power_constraint(len(candidate_set), K, h_candidate, Pt, noise)
                if val > best_gain:
                    best_gain = val
                    best_port = port
        ...
        while len(selected) > N_selected:
            ...
            for idx in range(len(selected)):
                reduced_set = selected[:idx] + selected[idx+1:]
                h_reduced = channel[reduced_set, :]
                val = sinr_balancing_power_constraint(len(reduced_set), K, h_reduced, Pt, noise)
                if val < worst_gain:
                    worst_gain = val
                    worst_idx = idx
