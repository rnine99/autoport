...
def select_ports(K, N_selected, N_Ports, Pt, n, H, noise):
   ...
   Select N_selected out of N_Ports ports to maximize the average objective value of n communications channels.
    ...
    for i in range(n):
        ...
        while improved:
            improved = False
            h_selected = H[i, selected, :]
            f_current = sinr_balancing_power_constraint(N_selected, K, h_selected, Pt, noise)
            
            worst_gain = float('inf')
            worst_idx = -1
            for idx, port in enumerate(selected):
                if norms[port] < worst_gain:
                    worst_gain = norms[port]
                    worst_idx = idx
            ...
            for new_port in unselected:
                ...
                if f_candidate > best_f:
                    best_f = f_candidate
                    best_unselected = new_port
            
            if best_unselected is not None:
                selected[worst_idx] = best_unselected
                improved = True
