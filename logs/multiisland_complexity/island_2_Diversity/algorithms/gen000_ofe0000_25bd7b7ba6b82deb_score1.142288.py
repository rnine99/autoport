...
def select_ports(K, N_selected, N_Ports, Pt, n, H, noise):
   ...
   Select N_selected out of N_Ports ports to maximize the average objective value of n communications channels.
    ...
    port_scores = np.zeros(N_Ports)
    
    for port in range(N_Ports):
        vals = []
        for i in range(n):
            h_single = H[i, port:port+1, :]
            val = sinr_balancing_power_constraint(1, K, h_single, Pt, noise)
            vals.append(val)
        vals_sorted = np.sort(vals)[::-1]
        weights = np.log2(1 + np.arange(len(vals_sorted)) + 1)[::-1]
        port_scores[port] = np.sum(vals_sorted * weights)
