...
def select_ports(K, N_selected, N_Ports, Pt, n, H, noise):
   ...
   Select N_selected out of N_Ports ports to maximize the average objective value of n communications channels.
    ...
    for i in range(n):
        channel = H[i]
        metric = np.zeros(N_Ports)
        for port in range(N_Ports):
            h_single = channel[port:port+1, :]
            metric[port] = sinr_balancing_power_constraint(1, K, h_single, Pt, noise)
        selected = np.argsort(metric)[-N_selected:].tolist()
        improved = True
        while improved:
            improved = False
            for idx in range(N_selected):
                ...
                best_val = sinr_balancing_power_constraint(N_selected, K, channel[selected, :], Pt, noise)
                for candidate in range(N_Ports):
                    if candidate in selected:
                        continue
                    temp = selected.copy()
                    temp[idx] = candidate
                    val = sinr_balancing_power_constraint(N_selected, K, channel[temp, :], Pt, noise)
                    if val > best_val:
                        best_val = val
                        best_swap = candidate
