...
def select_ports(K, N_selected, N_Ports, Pt, n, H, noise):
   ...
   Select N_selected out of N_Ports ports to maximize the average objective value of n communications channels.
    ...
    for i in range(n):
        # Initial selection: choose ports with largest channel norm
        norms = np.linalg.norm(H[i], axis=1)
        sorted_ports = np.argsort(norms)[::-1]
        selected = list(sorted_ports[:N_selected])
        
        # Local improvement via random swaps
        for _ in range(100):
            # Randomly pick one selected and one unselected port
            unselected = [p for p in range(N_Ports) if p not in selected]
            if not unselected:
                break
                
            idx_sel = np.random.randint(N_selected)
            idx_unsel = np.random.randint(len(unselected))
            
            old_port = selected[idx_sel]
            new_port = unselected[idx_unsel]
            
            # Test swap
            candidate = selected.copy()
            candidate[idx_sel] = new_port
            
            h_current = H[i, selected, :]
            h_candidate = H[i, candidate, :]
            
            f_current = sinr_balancing_power_constraint(N_selected, K, h_current, Pt, noise)
            f_candidate = sinr_balancing_power_constraint(N_selected, K, h_candidate, Pt, noise)
