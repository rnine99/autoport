...
def select_ports(K, N_selected, N_Ports, Pt, n, H, noise):
   ...
   Select N_selected out of N_Ports ports to maximize the average objective value of n communications channels.
    ...
    for i in range(n):
        beam_width = min(5, N_Ports - N_selected + 1)
        candidates = [([], 0.0)]
        
        for step in range(N_selected):
            new_candidates = []
            for selected, _ in candidates:
                available = [p for p in range(N_Ports) if p not in selected]
                for port in available:
                    ...
                    new_candidates.append((candidate_ports, f_val))
            
            new_candidates.sort(key=lambda x: x[1], reverse=True)
            candidates = new_candidates[:beam_width]
        
        best_ports, _ = max(candidates, key=lambda x: x[1])
