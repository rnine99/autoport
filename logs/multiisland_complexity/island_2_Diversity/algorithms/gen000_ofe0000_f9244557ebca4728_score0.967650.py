...
def select_ports(K, N_selected, N_Ports, Pt, n, H, noise):
   ...
   Select N_selected out of N_Ports ports to maximize the average objective value of n communications channels.
    ...
    for port in range(N_Ports):
        avg_gain = np.mean(np.abs(H[:, port, :])**2)
        min_gain = np.min(np.abs(H[:, port, :])**2)
        port_scores[port] = 0.7 * avg_gain + 0.3 * min_gain
    for i in range(n):
        channel_gains = np.sum(np.abs(H[i, :, :])**2, axis=1)
        sorted_ports = np.lexsort((-port_scores, -channel_gains))
        port_sample[i] = sorted_ports[:N_selected]
    return port_sample
