...
def select_ports(K, N_selected, N_Ports, Pt, n, H, noise):
   ...
   Select N_selected out of N_Ports ports to maximize the average objective value of n communications channels.
    ...
    port_importance = np.zeros(N_Ports)
        ...
        for _ in range(N_selected):
            ...
            for port in range(N_Ports):
                if port in current_set:
                    continue
                candidate = current_set + [port]
                h_eff = H[i, candidate, :]
                val = sinr_balancing_power_constraint(len(candidate), K, h_eff, Pt, noise)
                gain = val - (sinr_balancing_power_constraint(len(current_set), K, H[i, current_set, :], Pt, noise) if current_set else 0)
                if gain > best_gain:
                    best_gain = gain
                    best_port = port
            if best_port >= 0:
                gains[best_port] += best_gain
                current_set.append(best_port)
        port_importance += gains
    
    ranked_ports = np.argsort(port_importance)[::-1]
    ...
    for i in range(n):
        ...
        for port in ranked_ports:
            ...
            h_eff = H[i, candidate, :]
            f_val = sinr_balancing_power_constraint(len(candidate), K, h_eff, Pt, noise)
            if len(selected) == 0 or f_val > sinr_balancing_power_constraint(len(selected), K, H[i, selected, :], Pt, noise):
                selected.append(port)
        port_sample[i] = selected[:N_selected]
